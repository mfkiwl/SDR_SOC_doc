...
#define DDS_F_MAX 50000000	// Hz
#define DDS_F_MIN 7000000	// Hz
#define DDS_S_MAX 10000000	// Hz
#define DDS_S_MIN 10		// Hz
...
void LCD_Update(double freq, double step)
{
	...
	uint32_t i,j;
	int32_t fr = (int32_t) freq;
	int32_t s = (int32_t) step;
	uint8_t flag = 1;

	fr =	(fr-6000000 < 0) ? 6000000-fr : fr-6000000;
...
int main(void)
{
    ...
    while(1)
    {
    	while ((state == (((PIND & (1<<PD3)) | (PIND & (1<<PD2))) >> 2)) && ((PINB & (1<<PB2)))) // počkej dokud se neotočí
    		;
		...
		else // není stusknuto tlačítko
		{
			...
			if ( (old_state == 3 && state == 2)) //otočení směr plus
			{
				...
				{
					if ((f_lad+step) <= DDS_F_MAX && (f_lad+step) > f_lad)
					{
						f_lad += step;
						f += step;
						dds_SetFreq(f);
			...
			else // otočení směr minus
			{
				...
				if (mode == 0) // mód lazení frekvence
				{
					if ((f_lad-step) >= DDS_F_MIN && (f_lad-step) < f_lad)
					{
						f_lad -= step;
						f -= step;
						dds_SetFreq(f);
